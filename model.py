...
class Generator(object):
    ...
    def _add_placeholders(self):
        ...
        self._enc_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.max_enc_num, hps.max_enc_steps], name='enc_batch')
        self._enc_lens = tf.placeholder(tf.int32, [hps.batch_size],  name='enc_lens')
        self._enc_sen_lens = tf.placeholder(tf.int32, [hps.batch_size,hps.max_enc_num],  name='enc_sen_lens')
    ...
    def _add_encoder(self, encoder_inputs, seq_len, enc_len):
        ...
        encoder_inputs = tf.reshape(encoder_inputs, [self._hps.batch_size*self._hps.max_enc_num, self._hps.max_enc_steps, self._hps.emb_dim])
        seq_len = tf.reshape(seq_len,[self._hps.batch_size*self._hps.max_enc_num])
        ...
        hir_encoder_input = tf.reshape(hir_encoder_input, [self._hps.batch_size, self._hps.max_enc_num, 2*self._hps.hidden_dim])

        with tf.variable_scope('hir_encoder'):
            cell_fw_ = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init,
                                              state_is_tuple=True)
    ...
    def _add_decoder(self, loop_function, loop_function_max, input, attention_state
        ...
        input = tf.reshape(input, [hps.batch_size, hps.max_dec_steps, hps.emb_dim])
        input = tf.unstack(input, axis=1)
